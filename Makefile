...
process-traffic-hits:
	@make docker-run SERVICE=php-cli CMD="echo \" Modify this recipe so it calls your implementation of the traffic hit processor\""
...
create-traffic-type-in:
	make docker-run SERVICE=aws-cli CMD="--endpoint-url=${LOCALSTACK_ENDPOINT} lambda invoke --function-name dev-traffic-agent --cli-binary-format raw-in-base64-out --payload '{\"queryStringParameters\": {\"type\": \"type-in\", \"ip\": \"127.0.0.1\"}}' /dev/stdout"
